...
These are canonical role names — the actual label strings used in the issue tracker may differ. The mapping should have been provided to you - run `/idd:setup-matt-pocock-skills` if not.
...
4. **Grill (if needed).** If the issue needs fleshing out, run a `/idd:grill-with-docs` session.
...
Run `/clear` to reset the context, then `/idd:parallel-issue-runner #<prd>` to kick off parallel agent execution.
```
